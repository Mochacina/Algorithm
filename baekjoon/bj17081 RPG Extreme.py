class Hero:
    def __init__(self):
        ...
        self.armor = None
        self.accessories = []
        self.position = tuple([0, 0])  # Fix: Pass the position as a single argument to the tuple() function
        self.terrain = '.'
...
def move_hero(game_map, moves):
    ...
    for move in moves:
        ...
        for i in range(len(grid)):
            if i == hero.position[0]:
                print(''.join(grid[i][:hero.position[1]]+['@']+grid[i][hero.position[1]+1:]))
            else:
                print(''.join(grid[i]))
...
for i in range(N):
    for j in range(M):
        if grid[i][j] == '@':
            hero.position = tuple([i, j])
hero_moves = input()
while 1:
    try:
        x, y, *l = input().split()
        if len(l) == 5:
            monsters.append(Monster(l[0], int(l[1]), int(l[2]), int(l[3]), int(l[4]), tuple([int(x), int(y)])))
        elif len(l) == 2:
            item_boxes.append(ItemBox(l[0], l[1], tuple([int(x), int(y)])))
    except: break
hero.show()
